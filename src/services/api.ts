...
export const apiService = {
  ...
  async getTasks(filters: any = {}) {
    ...
    try {
      ...
      fetchError = e1;
      console.warn('[API] getTasks Tier 1 fetch unsuccessful, attempting Tier 2 (status only)...', e1.message || e1);
      ...
      try {
        ...
        console.warn('[API] getTasks Tier 2 fetch unsuccessful, attempting Tier 3 (unfiltered list, filter client-side)...', e2.message || e2);
        ...
        try {
          ...
          console.error('[API] getTasks Tier 3 fetch failed completely:', e3.message || e3);
          return [];
        }
      }
    }
    ...
    try {
      ...
      if ('error' in user) return decodedDocs;

      return decodedDocs.filter((task: any) => {
  ...
  async getNotifications() {
    ...
    try {
      ...
      console.warn('[API] getNotifications failed (possibly missing collection), using localStorage fallback:', e);
    }
